...
public class CreateTournamentUseCase implements CreateTournamentPort {
    ...
    public Tournament create(Tournament t, Long organizerId) {
        ...
        if (t.getMinParticipantsPerTeam() <= 0 || t.getMaxParticipantsPerTeam() <= 0) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "min/maxParticipantsPerTeam deben ser > 0");
        }
        if (t.getMinParticipantsPerTeam() > t.getMaxParticipantsPerTeam()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "minParticipantsPerTeam > maxParticipantsPerTeam");
        }

        if (t.isPrivateTournament() && (t.getPassword() == null || t.getPassword().isBlank())) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "password requerido para torneo privado");
        }
        ...
        if (deadline != null && start != null && deadline.after(start)) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "registration_deadline > startAt");
        }
        ...
        if (minT > 0 && maxT > 0 && minT > maxT) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "minParticipantsPerTournament > maxParticipantsPerTournament");
        }
        ...
        // Para no forzar ese mapeo ahora, pasamos organizerId al puerto de persistencia.
